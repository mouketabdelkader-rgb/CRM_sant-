...
def creer_base_de_donnees():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor = conn.cursor()
        cursor.execute('DROP TABLE IF EXISTS activites')
        cursor.execute('DROP TABLE IF EXISTS structures')
        cursor.execute('DROP TABLE IF EXISTS professionnels')

        cursor.execute('''
        CREATE TABLE professionnels (
            id_pp TEXT PRIMARY KEY, nom TEXT, prenom TEXT, date_premiere_apparition DATE,
            date_derniere_apparition DATE, est_actif BOOLEAN
        )''')
        cursor.execute('''
        CREATE TABLE structures (
            id_structure TEXT PRIMARY KEY, type_id TEXT, nom_structure TEXT, date_creation DATE,
            etat_administratif TEXT, nom_dirigeant TEXT, annee_naissance_dirigeant INTEGER,
            adresse_normalisee TEXT, latitude REAL, longitude REAL, date_dernier_enrichissement DATETIME
        )''')
        cursor.execute('''
        CREATE TABLE activites (
            id_activite INTEGER PRIMARY KEY AUTOINCREMENT, id_pp TEXT, id_structure TEXT, profession TEXT,
        ...
        print("Structure de la base de données (ré)initialisée.")
...
def telecharger_et_dezipper():
    ...
    with zipfile.ZipFile(ZIP_PATH, 'r') as zip_ref:
        ...
        if not txt_filename: raise FileNotFoundError("Fichier 'Personne_activite' introuvable.")
...
def integrer_donnees(txt_filename, date_maj_str):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        for chunk in reader:
            ...
            # On crée la colonne 'id_structure_final' en choisissant la bonne source
            chunk['id_structure_final'] = np.where(
                chunk["Libellé mode exercice"] == 'Lib,indép,artis,com',
                chunk["Numéro SIRET site"],
                chunk["Identifiant technique de la structure"]
            )

            # On ne garde que les lignes où l'identifiant de structure final n'est pas vide
            chunk.dropna(subset=['id_structure_final'], inplace=True)

            pros_data = chunk[["Identifiant PP", "Nom d'exercice", "Prénom d'exercice"]].drop_duplicates(subset=['Identifiant PP'])
            # On utilise notre nouvelle colonne pour peupler la table des structures
            structures_data = chunk[['id_structure_final']].dropna().drop_duplicates()
            # On utilise aussi notre nouvelle colonne pour la table des activités
            activites_data = chunk[["Identifiant PP", "id_structure_final", "Libellé profession", "Libellé mode exercice"]]
            ...
            cursor.executemany(f'''
                ...
                ON CONFLICT(id_pp) DO UPDATE SET date_derniere_apparition = '{date_maj_str}', est_actif = TRUE;
...
if __name__ == "__main__":
    creer_base_de_donnees() # On recrée la BDD de zéro pour être sûr
    nom_fichier_txt = telecharger_et_dezipper()
    date_du_jour = datetime.now().strftime('%Y-%m-%d')
    integrer_donnees(nom_fichier_txt, date_du_jour)
    
    print("\n--- Processus terminé avec succès ---")
    print(f"La base de données '{DB_NAME}' a été reconstruite correctement.")
